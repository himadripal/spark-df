...
object AppleMain extends  Util{
  ...
  def main(args : Array[String]) = {
    ...
    val w = Window.orderBy(desc("net_total"))
    val per_customer_sale = sales_and_refund_2013_may.groupBy(SALES.CUSTOMER_ID.toString)
                                                    ...
                                                    .withColumn("dense_rank", dense_rank().over(w)).where("dense_rank==2").join(customer_df,"customer_id")
                                                    .select(concat_ws(" ",$"customer_first_name",$"customer_last_name"),$"net_total")show()
